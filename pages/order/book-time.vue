        ...
        <view v-for="(item, index) in list" :key="index">
            ...
            <view v-if="item.isShow" class="uni-flex uni-row">
                <view class="uni-flex-item" style="margin: 0 30rpx;" v-for="(bt, i) in item.businessTime" :key="i" @click="changeCheck(bt, item.date)">
                    <view v-if="bt.isOrder" class="uni-flex uni-column" :class="bt.orderCount > 0 ? 'bt-active' : ''">
    ...
    export default {
        ...
        methods: {
            ...
            generateConfigList(restConfig, orderMenu) {
                ...
                this.getOrderCount(list, restConfig)
            },
            /**
             * 生成营业时间菜单
             * 
                ...
                if (list.length > 0) {
                    this.$httpForm.post('/wechat/order/count.json', {
                        ...
                        if (res.data.code === 'SUCCESS') {
                            const orderCount = res.data.rows || []
                            that.checkOrderCount(list, restConfig, orderCount)
                        }
                    })
                }
            },
            checkOrderCount(list, restConfig, orderCount) {
                let that = this
                list.forEach((item) => {
                    let businessTime = item.businessTime
                    businessTime.forEach((bt) => {
                        bt.orderCount = 0
                        const temp = orderCount.find(function(ele) {
